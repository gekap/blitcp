...
"""blitcp — PySide6 modern GUI.

A faithful native reproduction of the blitcp modern look-and-feel mockup:
...
def _ensure_std_streams():
    ...
    on Windows) have NO console, so sys.stdout/stderr are None — and blitcp.py
...
# Reuse the real blitcp engine for credentials + remote listing when it sits
...
try:
    import blitcp as fc
    # blitcp installs a CLI-style excepthook on import; restore the default so
    # the GUI behaves normally.
    sys.excepthook = sys.__excepthook__
    FC_OK = True
except Exception:
    try:
        # Pre-rename engine sitting beside an updated GUI still loads.
...
# The GUI shares the engine's gettext domain. Without the engine (demo mode)
# strings simply render in English. fc._ is a live function, so a later
# fc.set_language() re-routes every subsequent lookup.
I18N_CHOICES = [("en", "English"), ("el", "Ελληνικά"), ("zh_CN", "中文"),
                ("de", "Deutsch"), ("it", "Italiano"), ("es", "Español"),
                ("ja", "日本語")]

if FC_OK:
    # getattr-guarded: a pre-rename engine loaded through the fallback below
    # (fast-copy ≤3.12.x) predates i18n and has no _tr/set_language. The GUI
    # must still RUN with it — English strings, not a crash.
    def _tr(s):
        f = getattr(fc, "_tr", None)
        return f(s) if f else s

    def set_language(lang):
        f = getattr(fc, "set_language", None)
        if f:
            f(lang)
else:
    def _tr(s):
...
def _early_gui_language():
    """Apply the saved GUI language BEFORE any widget text is built.
    Mirrors BlitcpGUI._config_dir()/_load_gui_settings(), which need an
    instance that doesn't exist yet at this point in startup."""
    try:
        import json
        if sys.platform.startswith("win"):
        ...
        # Read-only here — new config dir first, pre-rename one as fallback
        # (the instance _config_dir migrates it on first write).
        lang = None
        for d in ("blitcp", "fast_copy"):
            try:
                with open(os.path.join(base, d, "gui_settings.json")) as f:
...
class PasswordDialog(QDialog):
    ...
    def __init__(self, parent, title, sub, label, allow_empty=False,
                 confirm=False, generator=False):
        super().__init__(parent)
        self.allow_empty = allow_empty
        self.confirm = confirm
        # Offer a diceware generate button only when SETTING a passphrase and
        # the engine provides the generator (a pre-rename engine may not).
        self.generator = (generator and confirm and FC_OK
                          and hasattr(fc, "generate_passphrase"))
        ...
        if confirm:
            lay.addSpacing(12)
            ll2 = QLabel(_tr("Confirm passphrase"))
        ...
        row = QHBoxLayout()
        if self.generator:
            gen = textbtn(_tr("Generate"), "bolt", oid="btn")
            gen.setToolTip(_tr("Fill in a random 6-word passphrase (~77 bits)"))
            gen.clicked.connect(self._generate)
            row.addWidget(gen)
        row.addStretch(1)
        cancel = textbtn(_tr("Cancel"), oid="btn")
        cancel.clicked.connect(self.reject)
        ok = textbtn(_tr("Confirm"), "lock-open", oid="btnprimary")
    ...
    def _generate(self):
        """Fill both fields with a fresh diceware passphrase and reveal it —
        the user must be able to read it to store it in a password manager."""
        phrase, bits = fc.generate_passphrase()
        for w in (self.input, self.input2):
            if w is not None:
                w.setText(phrase)
                w.setEchoMode(QLineEdit.Normal)
        self.err.setStyleSheet("color:#30a46c;")
        self.err.setText(_tr("Generated ({0} bits). Save it in your password "
                             "manager now — it is shown only here.").format(bits))
        self.err.show()
    ...
    def _fail(self, msg):
        self.err.setStyleSheet("color:#e5484d;")  # back to red after a green Generate note
...
class ConnectionDialog(QDialog):
    ...
    def __init__(self, parent, name, conn):
        ...
        cancel = textbtn(_tr("Cancel"), oid="btn")
        cancel.clicked.connect(self.reject)
        save = textbtn(_tr("Save"), "device-floppy", oid="btnprimary")
...
class TextPromptDialog(QDialog):
    ...
    def __init__(self, parent, title):
        ...
        cancel = textbtn(_tr("Cancel"), oid="btn")
        cancel.clicked.connect(self.reject)
        ok = textbtn(_tr("Add"), oid="btnprimary")
...
class RemoteBrowseDialog(QDialog):
    ...
    def __init__(self, gui, kind, start_conn=None):
        ...
        self.selected = None
        self.setModal(True)
        self.setWindowTitle(_tr("Browse SSH") if kind == "ssh" else _tr("Browse cloud"))
        self.resize(580, 580)

        lay = QVBoxLayout(self)
        lay.setContentsMargins(20, 18, 20, 18)
        lay.setSpacing(0)
        title = QLabel(_tr("Browse SSH server") if kind == "ssh" else _tr("Browse cloud storage"))
        title.setObjectName("sheettitle")
        lay.addWidget(title)
        lay.addSpacing(12)
        ...
        cancel = textbtn(_tr("Cancel"), oid="btn")
        cancel.clicked.connect(self.reject)
        self.okbtn = textbtn(_tr("Select this path"), "check", oid="btnprimary")
    ...
    def _mk_item(self, name, meta, up=False):
        ...
        fn.setObjectName("fn")
        fs = QLabel("" if up else (_tr("folder") if is_dir else meta))
        fs.setObjectName("fs")
        il.addWidget(fi)
        il.addWidget(fn, 1)
        il.addWidget(fs)
...
class UpdateDialog(QDialog):
    ...
    def __init__(self, parent, latest, current, releases, can_install, theme=None):
        ...
        self.setModal(True)
        self.setWindowTitle(_tr("Update available"))
        ...
        title = QLabel(f"blitcp {latest} is available")
        title.setObjectName("sheettitle")
        lay.addWidget(title)
        lay.addSpacing(4)
        ...
        row = QHBoxLayout()
        close = textbtn(_tr("Close"), oid="btn")
        close.clicked.connect(self.reject)
        row.addWidget(close)
        row.addStretch(1)
        dlonly = textbtn(_tr("Download only"), "arrow-down-circle", oid="btn")
        dlonly.clicked.connect(lambda: self._choose(self.DOWNLOAD_ONLY))
        row.addWidget(dlonly)
        install = textbtn(_tr("Download & install"), "arrow-down-circle", oid="btnprimary")
...
class _ConfirmHostKeyPolicy:
    ...
    def missing_host_key(self, client, hostname, key):
        ...
        try:
            ...
            m.setWindowTitle(_tr("Unknown SSH host"))
...
# ───────────────────────────────────────────────────────── main window ──
class BlitcpGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.dark = True       # dark is the default theme
        self.setWindowTitle("blitcp — " + GUI_VERSION)
        ...
        self.creds_tamper = False        # file bound to a different blitcp.py
    ...
    def _init_credentials(self):
        ...
        one locked (unlocked on first visit to Connections). No blitcp.py or
        ...
        if fc._is_encrypted(raw):
            ...
            # vs this blitcp.py — visible even while locked.
            try:
    ...
    def unlock_credentials(self):
        ...
        if not FC_OK or self.creds_loaded or not self.creds_encrypted:
            return
        dlg = PasswordDialog(
            self, _tr("Unlock credentials.json"), "", _tr("Enter passphrase"))
        if not dlg.exec():
            return
        ...
        try:
            ...
            self.show_toast(_tr("Could not read credentials:") + " " + str(e).splitlines()[0])
            return
    ...
    def _build_trees(self):
        ...
        self.local_tree = {
            ...
            "/C:/projects": [["backups", "dir"], ["blitcp", "dir"]],
    ...
    def _build_sidebar(self):
        ...
        brand.setSpacing(8)
        logo_png = os.path.join(HERE, "assets", "blitcp-logo.png")
        ...
        self.nav_items = {}
        items = [
            ("transfer", "arrows-exchange", _tr("Transfer")),
            ("connections", "key", _tr("Connections")),
            ("cloud", "folders", _tr("Browse files")),
            ("history", "history", _tr("History")),
            ("settings", "settings", _tr("Settings")),
        ]
        ...
        self.doc_btn = QPushButton("  " + ic("book") + "   " + _tr("Documentation"))
        ...
        self.theme_btn = QPushButton(
            "  " + (ic("sun") + "   " + _tr("Light mode") if self.dark
                    else ic("moon") + "   " + _tr("Dark mode")))
    ...
    def _screen_transfer(self, lay):
        self._header(lay, _tr("New transfer"),
                     _tr("Many sources → one destination. With more than one "
                         "source, local only."))

        # sources label + multi pill
        lblrow = QHBoxLayout()
        srclbl = QLabel(_tr("SOURCES"))
        srclbl.setObjectName("lbl")
        self.multi_pill = QLabel(ic("folder") + " " + _tr("local only when multiple"))
        self.multi_pill.setObjectName("pill")
        self.multi_pill.hide()
        lblrow.addWidget(srclbl)
        lblrow.addSpacing(8)
        srchint = QLabel(_tr("example — edit before running"))
        ...
        add = textbtn(ic("plus") + " " + _tr("Add source"), oid="add")
        ...
        # destination
        drow = QHBoxLayout()
        dl = QLabel(_tr("DESTINATION"))
        dl.setObjectName("lbl")
        dh = QLabel(_tr("single · local / SSH / cloud"))
        dh.setObjectName("sub")
        drow.addWidget(dl)
        drow.addSpacing(8)
        drow.addWidget(dh)
        drow.addSpacing(8)
        dhint = QLabel(_tr("example — edit before running"))
        ...
        self.dst_icon.setToolTip(_tr("Browse…"))
        ...
        for name, active in [("dedup", True), ("verify", True)]:
            o = self._opt(_tr(name), active)
            self.transfer_opts[name] = o
            opts.addWidget(o)
        hlbl = QLabel(_tr("hash"))
        ...
        self.start_btn = textbtn(_tr("Start transfer"), "player-play", oid="btnprimary")
        self.start_btn.clicked.connect(lambda: self.run_transfer(False))
        self.preview_btn = textbtn(_tr("Preview plan"), "eye", oid="btn")
    ...
    def _build_advanced(self):
        ...
        lay.addWidget(self._gl(_tr("COPY")))
        lay.addSpacing(9)
        grid = QGridLayout()
        ...
        lay.addWidget(self._gl(_tr("PRESERVE METADATA")))
        lay.addSpacing(9)
        meta = QHBoxLayout()
        meta.setSpacing(8)
        self.meta_opts = {}
        ...
        lay.addWidget(self._gl(_tr("FLAGS")))
        lay.addSpacing(9)
        flags = QHBoxLayout()
        flags.setSpacing(8)
        self.flag_opts = {}
        ...
        lay.addWidget(self._gl(_tr("EXCLUDE PATTERNS")))
        lay.addSpacing(9)
        ...
        lay.addWidget(self._gl(_tr("INDEX EXISTING (reflink dedup vs pre-existing files)")))
        lay.addSpacing(9)
    ...
    def render_idx_chips(self):
        ...
        add = QPushButton(_tr("+ add folder…"))
        add.setObjectName("chipadd")
        add.setCursor(Qt.PointingHandCursor)
    ...
    def _build_progress(self):
        ...
        top = QHBoxLayout()
        self.pstate = QLabel(_tr("Ready"))
        self.pstate.setObjectName("pstate")
        self.pfiles = QLabel(_tr("0 / 0 files"))
    ...
    def _screen_connections(self, lay):
        self._header(lay, _tr("Connections"),
                     _tr("Read from / saved to credentials.json. Double-click "
                         "to edit."))
        addrow = QHBoxLayout()
        addrow.addStretch(1)
        self.reload_btn = textbtn(_tr("Reload"), "refresh", oid="btnsm")
        self.reload_btn.clicked.connect(self.reload_credentials)
        self.rekey_btn = textbtn(_tr("Rekey"), "key", oid="btnsm")
        self.rekey_btn.clicked.connect(self.rekey_credentials)
        addc = textbtn(_tr("Add connection"), "plus", oid="btnsmprimary")
    ...
    def _screen_cloud(self, lay):
        self._header(lay, _tr("Browse files"),
                     _tr("Folder = navigate · file = click to select · "
                         "double-click = set as source."))
        ...
        bts = QHBoxLayout()
        bts.setSpacing(9)
        setsrc = textbtn(_tr("Set as source"), "arrow-down-circle", oid="btn")
        setsrc.clicked.connect(self.browse_set_source)
        setdst = textbtn(_tr("Set as destination"), "arrow-up-circle", oid="btn")
    ...
    def _screen_history(self, lay):
        self._header(lay, _tr("History"),
                     _tr("Past transfers from the saved JSON logs."))
    ...
    def _screen_settings(self, lay):
        self._header(lay, _tr("Settings"),
                     _tr("Language, dependencies, version, updates."))

        ll = QLabel(_tr("LANGUAGE"))
        ll.setObjectName("sectl")
        lay.addWidget(ll)
        lay.addSpacing(8)
        lang_card = QFrame()
        lang_card.setObjectName("conn")
        lcl = QHBoxLayout(lang_card)
        lcl.setContentsMargins(14, 12, 14, 12)
        lcl.setSpacing(13)
        lic = QLabel(ic("world"))
        lic.setObjectName("connicon")
        llab = QLabel(_tr("Interface language"))
        llab.setObjectName("connname")
        self.lang_sel = QComboBox()
        self.lang_sel.setObjectName("mini")
        for code, native in I18N_CHOICES:
            self.lang_sel.addItem(native, code)
        saved_lang = self._load_gui_settings().get("language", "en")
        idx = next((i for i, (c, _n) in enumerate(I18N_CHOICES)
                    if c == saved_lang), 0)
        self.lang_sel.setCurrentIndex(idx)
        self.lang_sel.currentIndexChanged.connect(self._on_language_changed)
        lcl.addWidget(lic)
        lcl.addWidget(llab, 1)
        lcl.addWidget(self.lang_sel)
        lay.addWidget(lang_card)
        lay.addSpacing(14)

        dl = QLabel(_tr("DEPENDENCIES"))
    ...
    def _dep_card(self, icon, name, meta, n_have, n_total, missing):
        ...
        if missing:
            ...
            btn = textbtn(_tr("Install"), oid="btnsm")
            btn.clicked.connect(lambda _=False, m=list(missing): self._pip_install_deps(m))
            crow.addWidget(btn)
            outer.addLayout(crow)
        return card

    # ───────────────────────────────── core engine (blitcp.py) ──
    def _ensure_core(self):
        """If the GUI was launched WITHOUT blitcp.py next to it (and isn't a
        bundled build), offer to download the version that matches this GUI."""
        if FC_OK or getattr(sys, "frozen", False):
            return
        m = QMessageBox(self)
        m.setWindowTitle(_tr("Core engine missing"))
        m.setIcon(QMessageBox.Warning)
        m.setText(_tr("blitcp.py (the copy engine) was not found next to the GUI."))
        m.setInformativeText(
    ...
    def _download_core(self):
        import urllib.request
        self.show_toast(_tr("Downloading blitcp.py") + " v" + GUI_VERSION + " …")
        QApplication.processEvents()
        base = "https://raw.githubusercontent.com/" + GUI_REPO
        # blitcp.py on current tags; blitcp.py covers pre-rename tags.
        urls = [base + "/v" + GUI_VERSION + "/blitcp.py",
                base + "/" + GUI_VERSION + "/blitcp.py",
                base + "/main/blitcp.py",
                base + "/v" + GUI_VERSION + "/fast_copy.py",
                base + "/main/fast_copy.py"]
        dest = os.path.join(HERE, "blitcp.py")
        ...
        for u in urls:
            try:
                ...
                err = str(e)
        QMessageBox.critical(self, _tr("Download failed"),
                             _tr("Could not download blitcp.py:") + "\n" + err +
                             "\n\nGet it manually from:\n"
                             "https://github.com/" + GUI_REPO + "/releases")

    def _core_downloaded(self, dest):
        m = QMessageBox(self)
        m.setWindowTitle(_tr("Core engine installed"))
        m.setIcon(QMessageBox.Information)
        m.setText("Downloaded blitcp.py to:\n" + dest)
    ...
    def _pip_install_deps(self, specs):
        ...
        if getattr(sys, "frozen", False):
            ...
            try:
                ...
                copied = ""
            m = QMessageBox(self)
            m.setWindowTitle(_tr("Can't install into the packaged app"))
            m.setIcon(QMessageBox.Information)
            m.setText("This standalone blitcp build can't add Python "
                      "packages to itself at runtime.")
            m.setInformativeText(
                "To use these optional integrations, run blitcp from the "
                "Python source and install them into that environment:\n\n  "
                + cmd + copied)
            m.exec()
            return
        if getattr(self, "_pip_proc", None) is not None:
            self.show_toast(_tr("Install already running…"))
            return
        self.show_toast(_tr("Installing") + " " + ", ".join(specs) + " …")
        proc = QProcess(self)
        self._pip_proc = proc

        def _done(*_):
            ok = (proc.exitCode() == 0)
            self.show_toast(_tr("Installed ✓") if ok else _tr("Install failed — see terminal"))
            self._pip_proc = None
            self._render_deps()
            proc.deleteLater()
    ...
    def open_docs(self):
        import webbrowser
        webbrowser.open("https://blitcp.dev/#docs")
    ...
    def run_transfer(self, dry):
        ...
        if not FC_OK or not self._fc_script_path():
            self.show_toast(_tr("blitcp.py not found next to the GUI"))
            return
        ...
        if not sources:
            self.show_toast(_tr("Add at least one source"))
            return
        if not dest:
        ...
        if self.creds_pw:
            pw = bytes(self.creds_pw).decode("utf-8", "replace")
            env.insert("BLITCP_CREDS_PASSPHRASE", pw)
            # A pre-rename engine (fallback in _fc_script_path) only knows the
            # old name; the engine scrubs both from child environments.
    ...
    def _handle_out_line(self, line):
        ...
        if line.lstrip().startswith("{"):
            ...
            if ev.get("t") == "verify":
                ...
                self._stop_indeterminate()
                self.pstate.setText(_tr("Verifying…"))
                pct = ev.get("pct", 0) or 0
                self.barf.setFixedWidth(int(self.bar.width() * min(pct, 100) / 100))
                self.stat_vals["Progress"].setText(f"{round(pct)}%")
                self.pfiles.setText(
    ...
    def _on_proc_error(self, err):
        if not self.running:
            return
        self.running = False
        self.pstate.setText(_tr("Failed"))
    ...
    def _on_proc_finished(self, code, status):
        ...
        if dry:
            self.show_toast(_tr("Dry run complete") if ok else _tr("Dry run failed — see log"))
        elif ok:
    ...
    def _conn_card(self, name, c):
        ...
        if st == "ok":
            ...
            badge = QLabel(_tr("testing…"))
            badge.setObjectName("badgetest")
        else:
            badge = QLabel(_tr("untested"))
    ...
    def _render_creds_banner(self):
        ...
        if self.creds_tamper:
            ...
            wi.setObjectName("warnicon")
            wi.setAlignment(Qt.AlignTop)
            wt = QLabel(_tr("This <b>credentials.json</b> was bound to a different "
                            "blitcp.py (tamper check). If you just updated "
                            "blitcp, press <b>Rekey</b> to re-bind it."))
            wt.setObjectName("warntext")
            wt.setWordWrap(True)
            wt.setTextFormat(Qt.RichText)
            wt.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Preferred)
            rk = textbtn(_tr("Rekey"), "key", oid="btnsm")
        ...
        if self.creds_encrypted and not self.creds_loaded:
            ...
            col = QVBoxLayout()
            col.setSpacing(2)
            nm = QLabel(_tr("credentials.json is locked"))
            nm.setObjectName("connname")
            ...
            col.addWidget(nm)
            col.addWidget(mt)
            unlock = textbtn(_tr("Unlock"), "lock-open", oid="btnsmprimary")
            ...
            l = QLabel(ic("alert-triangle") + "  blitcp.py not found — showing demo connections")
            l.setObjectName("sub")
    ...
    def open_conn_modal(self, name):
        ...
        while dlg.exec():
            ...
            if rname != name and rname in self.conns:
                self.show_toast(_tr("Name already exists"))
                continue
            ...
            if not ok:
                ...
                self.show_toast(_tr("Could not save:") + " " + err)
                break
            self.render_conns()
            self.fill_conn_sel()
    ...
    def delete_conn(self, name):
        ...
        self.render_conns()
        self.fill_conn_sel()
        self.show_toast(_tr("Removed") + " " + name)
    ...
    def _persist_credentials(self):
        ...
        if self.creds_encrypted and not self.creds_pw:
            return False, _tr("Unlock credentials.json first")
        ...
        try:
            ...
            if hasattr(fc, "_creds_cache"):
                fc._creds_cache.clear()
            self.creds_tamper = False    # any save re-binds to this blitcp.py
            return True, None
        except SystemExit as e:
    ...
    def _offer_encryption(self):
        ...
        dlg = PasswordDialog(
            ...
            "New passphrase", confirm=True, generator=True)
        if not dlg.exec():
    ...
    def rekey_credentials(self):
        """Re-bind credentials.json to this blitcp.py (engine `creds rekey`).
        Re-encrypts with the SAME passphrase — does not change it."""
        if not FC_OK or not self.creds_path or not hasattr(fc, "_save_credentials_file"):
            self.show_toast(_tr("Rekey needs blitcp.py"))
            return
        if not (os.path.isfile(self.creds_path) and self.creds_encrypted):
            self.show_toast(_tr("credentials.json is not encrypted — nothing to rekey"))
            return
        ...
        if not ok:
            self.show_toast(_tr("Rekey failed:") + " " + err)
            return
        self.creds_tamper = False
        self.render_conns()
        self.show_toast(_tr("Re-bound credentials.json to this blitcp.py"))

    # ──────────────────────────────────────────────── file browser ──
    def fill_conn_sel(self):
        self.conn_sel.blockSignals(True)
        self.conn_sel.clear()
        self.conn_sel.addItem(_tr("This PC · local files"), "__local")
        for n, c in self.conns.items():
    ...
    def elevate_local(self):
        ...
        if sys.platform.startswith("win"):
            self.show_toast(_tr("Access denied — run as Administrator to browse this folder"))
            return
        dlg = PasswordDialog(
        ...
        if self._sudo_entries(self.cwd) is None:
            self._sudo_pw = None
            self.show_toast(_tr("sudo: authentication failed"))
            return
        self.show_toast(_tr("sudo authenticated"))
        self.render_browse()
    ...
    def _cloud_entries(self, conn, cwd):
        ...
        ctype = conn.get("type")
        if not FC_OK:
            return None, "blitcp.py is required for cloud browsing"
        is_smb = ctype == "smb"
    ...
    def _locked_folder_row(self):
        ...
        fn = QLabel(_tr("Permission denied — click to unlock with sudo"))
        fn.setObjectName("fn")
        fs = QLabel(_tr("locked"))
        fs.setObjectName("fs")
        il.addWidget(fi)
        il.addWidget(fn, 1)
        il.addWidget(fs)
    ...
    def _file_item(self, name, meta, up=False):
        ...
        fn = QLabel(name + ("/" if is_dir and not up else ""))
        fn.setObjectName("fn")
        fs = QLabel("" if up else (_tr("drive") if meta == "drive" else _tr("folder") if is_dir else meta))
        fs.setObjectName("fs")
        il.addWidget(fi)
        il.addWidget(fn, 1)
        il.addWidget(fs)
    ...
    def _dblclick_file(self, name):
        ...
        self.render_sources()
        self.show_toast(_tr("Added {0} as source").format(name))
    ...
    def browse_set_dest(self):
        ...
        self.render_dest()
        self.show_toast(_tr("Set as destination"))
    ...
    def _config_dir(self):
        ...
        An existing pre-rename blitcp dir (gui_settings.json, history/) is
        renamed in place on first touch — same migration as the engine's."""
        if sys.platform.startswith("win"):
            base = os.environ.get("APPDATA") or os.path.expanduser("~")
        else:
            base = os.environ.get("XDG_CONFIG_HOME") or os.path.join(
    ...
    def _write_history(self, record):
        ...
        try:
            ...
            self.show_toast(_tr("Could not save history:") + " " + str(e).splitlines()[0])
    ...
    def _hist_card(self, rec):
        ...
        if failed:
            badge = QLabel(ic("alert-triangle") + " " + _tr("failed"))
            badge.setObjectName("badgetest")
        again = textbtn(_tr("Run again"), "refresh", oid="btnsm")
    ...
    def _run_again(self, cfg):
        ...
        self.show_toast(_tr("Loaded transfer — review and Start"))

    def check_update(self):
        if not FC_OK or not hasattr(fc, "_fetch_releases"):
            self.show_toast(_tr("Update check needs blitcp.py"))
            return
        if getattr(self, "_upd_running", False):
            return
        self.upd_btn.setText(_tr("Checking…"))
    ...
    def _on_update_result(self, res):
        ...
        if res.get("err"):
            self.upd_btn.setText(_tr("Check for updates"))
            if not startup:
                self.show_toast(_tr("Could not check for updates")
                                + (": " + res["err"] if res["err"] != "network" else ""))
        elif res.get("uptodate"):
            self.upd_btn.setText(_tr("Up to date"))
            if not startup:
                self.show_toast(f"blitcp is up to date (v{res['uptodate']})")
        elif res.get("latest"):
    ...
    def _on_language_changed(self, idx):
        """Persist the language choice and offer an immediate restart — a
        full live retranslate would need every constructed widget re-texted,
        so a one-click relaunch is the honest way to apply it everywhere."""
        code = self.lang_sel.itemData(idx) or "en"
        s = self._load_gui_settings()
        s["language"] = code
        self._save_gui_settings(s)
        set_language(code)
        if getattr(self, "running", False):
            # never relaunch mid-transfer
            self.show_toast(_tr("Language saved — restart to apply everywhere"))
            return
        m = QMessageBox(self)
        m.setWindowTitle(_tr("Language"))
        m.setIcon(QMessageBox.Question)
        m.setText(_tr("Restart blitcp to apply the new language everywhere?"))
        r = m.addButton(_tr("Restart now"), QMessageBox.AcceptRole)
        m.addButton(_tr("Later"), QMessageBox.RejectRole)
        m.exec()
        if m.clickedButton() is r:
            if getattr(sys, "frozen", False):
    ...
    def download_update(self, install=True):
        ...
        if self.running:
            self.show_toast(_tr("Finish or cancel the running copy before updating"))
            return
        ...
        if not url:
            QMessageBox.warning(
                self, _tr("Update"),
                _tr("Couldn't find {asset} in release {tag}.").format(asset=asset, tag=tag) + "\n\n" + _tr("Download manually:") + "\n"
        ...
        if parsed.scheme != "https" or parsed.hostname not in allowed:
            QMessageBox.critical(self, _tr("Update"),
                                 _tr("Unexpected download URL (not HTTPS GitHub):") + "\n" + url)
            return
        ...
        self.upd_btn.setEnabled(False)
        self.upd_btn.setText(_tr("Downloading…"))
        self.show_toast(_tr("Downloading") + " " + asset + " …")
        ...
        if not ok:
            if target:                       # clean a partial .update_tmp
                try:
                    ...
                    pass
            QMessageBox.critical(self, _tr("Download failed"),
                                 _tr("Could not download the update:") + "\n" + info)
            return
        dest = info

        # In-place install (frozen Linux/Windows): replace the running binary
        # and offer to relaunch — the GUI equivalent of `blitcp --update`.
        if target:
            if self.running:                 # a copy started during the download
                self.show_toast(_tr("Copy in progress — update not applied"))
                try:
                    os.remove(dest)
                except OSError:
                    pass
            ...
            if not ok2:
                ...
                QMessageBox.critical(
                    ...
                    _tr("Could not replace the application:") + "\n" + msg)
                return
            self._prompt_relaunch()
            return

        if dest.endswith(".app.zip"):
            hint = ("Quit blitcp, unzip the file, and drag blitcp.app into "
                    "Applications (replacing the old one).")
        elif sys.platform.startswith("win"):
            hint = ("Close blitcp, then replace your current "
                    "blitcp_gui.exe with the downloaded file.")
        else:
            hint = ("Close blitcp, then replace your current binary with the "
                    "downloaded file (chmod +x it).")
        m = QMessageBox(self)
        m.setWindowTitle(_tr("Update downloaded"))
        m.setIcon(QMessageBox.Information)
    ...
    def _prompt_relaunch(self):
        ...
        m = QMessageBox(self)
        m.setWindowTitle(_tr("Update installed"))
        m.setIcon(QMessageBox.Information)
        m.setText(f"blitcp was updated to {tag}.")
...
def main():
    ...
    # blitcp.main() instead of launching the GUI. (No-op for plain .py runs,
    # which shell out to blitcp.py directly.)
    if len(sys.argv) > 1 and sys.argv[1] == "--fc-core":
        if not FC_OK:
            sys.stderr.write("blitcp engine not bundled\n")
            raise SystemExit(2)
    ...
    app.setApplicationName("blitcp")
    app.setApplicationDisplayName("blitcp")
    app.setDesktopFileName("blitcp")     # Wayland/GNOME taskbar association
    ...
    w = BlitcpGUI()
    w.show()
    # If blitcp.py isn't next to the GUI, offer to fetch the matching version.
